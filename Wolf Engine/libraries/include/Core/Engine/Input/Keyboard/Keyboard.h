...
namespace we {
	...
	public:
		Keyboard();
		~Keyboard();
		inline bool GetKeyDown(we::WE_KEYBOARD_BUTTON keyCode)      const { return m_KeyDown[keyCode]; }
		inline bool GetKeyUp(we::WE_KEYBOARD_BUTTON keyCode)        const { return m_KeyUp[keyCode]; }

		void Update();
	private:
		static const int NUM_KEYBOARD_KEYS = 284;
		bool m_KeyDown[NUM_KEYBOARD_KEYS];
		bool m_KeyUp[NUM_KEYBOARD_KEYS];
